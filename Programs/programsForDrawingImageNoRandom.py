...
# def draw_anterior_b_no_random(seglen, theta, gamma, phi, dh1, dh2, dimension, size_lut, randomize ,
#                               d_eye_r, c_eye_r, c_belly_r, c_head_r,
#                               eyes_br_r, belly_br_r, head_br_r,
#                               rand1_eye_r, rand2_eye_r, rand3_eye_r,
#                               rand1_belly_r, rand2_belly_r,
#                               rand1_head_r, rand2_head_r):
def draw_anterior_b_no_random(seglen, theta, gamma, phi, dh1, dh2, dimension, size_lut, randomize, randomnessVector = None):

    if randomnessVector is None:
        ...
        d_eye_r, c_eye_r, c_belly_r ,c_head_r ,eyes_br_r ,belly_br_r ,head_br_r , \
            rand1_eye_r ,rand2_eye_r ,rand3_eye_r ,rand1_belly_r ,rand2_belly_r ,\
            rand1_head_r ,rand2_head_r  = randomnessVector[:14]
    ...
    d_eye = seglen * (randomize * d_eye_r + (1 - randomize))
    # # Note: temporary
    # d_eye = seglen * 1.18
    # Relative position of eyes, belly and head on the ball and stick model
    c_eyes = 1.9 * (randomize * c_eye_r + (1 - randomize))
    c_belly = 0.98 * (randomize * c_belly_r + (1 - randomize))
    c_head = 1.04 * (randomize * c_head_r + (1 - randomize))
    ...
    eyes_br = 235 * (randomize * eyes_br_r + (1 - randomize))
    belly_br = eyes_br * 0.83 * (randomize * belly_br_r + (1 - randomize))
    head_br = belly_br * 0.64 * (randomize * head_br_r + (1 - randomize))
    # # # NOTE: temporary
    # eyes_br = 255 * .9
    # # belly_br = 255 * .
    # # head_br = 255 * .28
    ...
    rand1_eye = randomize * rand1_eye_r + (1 - randomize)
    rand2_eye = randomize * rand2_eye_r + (1 - randomize)
    rand3_eye = randomize * rand3_eye_r + (1 - randomize)
    rand1_belly = randomize * rand1_belly_r + (1 - randomize)
    rand2_belly = randomize * rand2_belly_r + (1 - randomize)
    rand1_head = randomize * rand1_head_r + (1 - randomize)
    rand2_head = randomize * rand2_head_r + (1 - randomize)
...
def gen_lut_b_tail_no_random(n, seglen, d1, d2, t, randomize, random_number_size_r = None):
    if random_number_size_r is None:
    ...
    random_number_size = randomize * random_number_size_r + (1 - randomize) * 0.5
...
def f_x_to_model(x, seglen, randomize):
    ...
    fish_anterior, eye1_c, eye2_c, _, _, _, _ = draw_anterior_b_no_random(seglen, x[2], 0, 0, dh1, dh2, 2, size_lut, randomize)
    ...
    for ni in range(0, 7):
        ...
        dt2 = pt[1, n] - coor_t2
        tailpix = imblank
        tail_model = gen_lut_b_tail_no_random(ni, seglen, dt1, dt2, theta[n], randomize, None)
        tailpix[
...
def f_x_to_model_evaluation(x, seglen, randomize, imageSizeX, imageSizeY):
    ...
    dh1 = pt[0, 1] - np.floor(pt[0, 1])
    dh2 = pt[1, 1] - np.floor(pt[1, 1])
    fish_anterior, eye1_c, eye2_c, _, _, _, _ = draw_anterior_b_no_random(seglen, x[2], 0, 0, dh1, dh2, 2, size_lut, randomize, None)
    imblank = np.zeros((int(imageSizeY), int(imageSizeX)), dtype=np.uint8)
    headpix = imblank.copy()
    bodypix = imblank.copy()
    coor_h1 = np.floor(pt[0, 1])
...
def f_x_to_model_centered(x, seglen, randomize, randomness_vector):
    ...
    dh1 = pt[0, 1] - np.floor(pt[0, 1])
    dh2 = pt[1, 1] - np.floor(pt[1, 1])
    fish_anterior, eye1_c, eye2_c, _, _, _, _ = draw_anterior_b_no_random(seglen, x[2], 0, 0, dh1, dh2, 2, size_lut, randomize, randomness_vector)
    ...
    for ni in range(0, 7):
        ...
        dt2 = pt[1, n] - coor_t2
        tailpix = imblank
        tail_model = gen_lut_b_tail_no_random(ni, seglen, dt1, dt2, theta[n], randomize, randomness_vector[14])
        tailpix[
        int(np.maximum(0, coor_t2 - (size_half - 1))): int(np.minimum(imageSizeY, 1 + coor_t2 + (size_half - 1))),
        int(np.maximum(0, coor_t1 - (size_half - 1))): int(
            np.minimum(imageSizeX, 1 + coor_t1 + (size_half - 1)))] = tail_model[
    ...
    # Combine headpix and tailpix into a single image
    # TODO: there is some extra randomness here, that did not get accounted
    graymodel = np.maximum(headpix, (randomness_vector[15] * randomize + (1 - randomize)) * bodypix)
    return graymodel, pt, new_origin


def f_x_to_model_bigger(x, seglen, randomize, imageSizeX, imageSizeY, randomness_vector):
    """
    ...
    """
    graymodel, pt, new_origin = f_x_to_model_centered(x, seglen, randomize, randomness_vector)
